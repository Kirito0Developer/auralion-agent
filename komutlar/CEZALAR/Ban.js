...
exports.run = async (client, message, args) => {
  ...
  if (seçim == "bilgi") {
    ...
    let arr2 = await rolAyarlarDB.get(`rolayarlar.ban_users`) || ["683707976468398227"];
    ...
    let embed = new MessageEmbed()
      ...
      .addField(`<:asgard_mavi_parlak_yildiz:939653590132351077> **Nasıl kullanılır?**`, `Ban Atmak için \`${prefix[0]}ban @etiket [sebep]\` komutunu kullan.`)
      .addField(`<:asgard_dev:939265889528873021> **Cezayı nasıl kaldıracağım?**`, `aili kaldırmak için \`${prefix[0]}unban ID\` komutunu kullan.`)
      .addField("<:asgard_kalkan:939265715872088096> **Komutu kimler kullanabilir?**", "Bu komutu **Yönetici** - **Üyeleri Yasakla** ya da **Ban Sorumlusu** yetkisine sahip olan kişiler kullanabilir.")
      .addField(`<:asgard_dev:939265889528873021> **Belirlediğim rollere koyabiliyor muyum?**`, `Eğer belirlediğin rollere koymak istiyorsan bunun için \n\`${prefix[0]}ban yetkiler [roller/idler]\` komutunu kullanmalısın.`)
      .addField(`<:asgard_dev:939265889528873021> **Belirlediğim kişileri koyabiliyor muyum?**`, `Eğer belirlediğin kişileri koymak istiyorsan bunun için \n\`${prefix[0]}ban kişiler [kişiler/idler]\` komutunu kullanmalısın.`)
      .addField(`<:asgard_mangekyou_sharingan:939265745580351509> **Belirlediğim kanala loglatabiliyor muyum?**`, `Eğer belirlediğin kanala loglatmak isteiyorsan bunun için \n\`${prefix[0]}ban kanal [kanal/id]\` komutunu kullanmalısın.`)
  ...
  if (message.member.permissions.has("ADMINISTRATOR") && seçim != "bilgi") {
    if (seçim == "yetkiler") {
      ...
      if (roles.length <= 0)
        return await message.channel.send(new MessageEmbed()
          .setColor("#FF6553")
          .setTitle("<:asgard_carpii:939960104759996547> Komutu Yanlış Kullandın! <:asgard_carpii:939960104759996547>")
          .setDescription(`Sanırsam rolleri eklemeyi unuttun bu komut hakkında bilgi edinmek için \`${prefix[0]}ban bilgi\` kullanmayı dene.`)
          .addField(`<:asgard_tikk:939960105762435222> Örnek`, `\`\`\`${prefix[0]}ban yetkiler @rol1 @rol2 @rol3\`\`\``, true)
          .setThumbnail("https://cdn.discordapp.com/attachments/698398041518112898/699064766626267188/giphy.gif")
          .addField(`<:asgard_tikk:939960105762435222> Örnek`, `\`\`\`${prefix[0]}ban yetkiler ID1 ID2 ID3\`\`\``, true)
          .setFooter(message.author.username + " Tarafından kullanıldı.", message.author.avatarURL({
            dynamic: true
          })));
      ...
      let embed = new MessageEmbed()
        ...
        .addField("<:asgard_kalkan:939265715872088096> Yetkiler", message.guild.roles.cache.filter(r => roles.some(d => d == r.id)).map(r => r.id).map((r, index) => (index + 1) + "- " + message.guild.roles.cache.get(r).toString()).join("\n"))
      return await message.channel.send(embed);
    }
    if (seçim == "kanal") {
      ...
      .addField(`<:asgard_onay_rozet:939959006657663006> Örnek`, `\`\`\`${prefix[0]}ban kanal #kanal\`\`\``, true)
      .setThumbnail("https://cdn.discordapp.com/attachments/698398041518112898/699064766626267188/giphy.gif")
      .addField(`<:asgard_onay_rozet:939959006657663006> Örnek`, `\`\`\`${prefix[0]}ban kanal ID\`\`\``, true)
      .setFooter(message.author.username + " Tarafından kullanıldı.", message.author.avatarURL({
        dynamic: true
      })));
    ...
    if (seçim == "kişiler") {
      ...
      .addField(`<:asgard_mavi_parlak_yildiz:939958604532940871> Örnek`, `\`\`\`${prefix[0]}ban kişiler @etiket1 @etiket2\`\`\``, true)
      .setThumbnail("https://cdn.discordapp.com/attachments/698398041518112898/699064766626267188/giphy.gif")
      .addField(`<:asgard_mavi_parlak_yildiz:939958604532940871> Örnek`, `\`\`\`${prefix[0]}ban kişiler ID1 ID2\`\`\``, true)
      .setFooter(message.author.username + " Tarafından kullanıldı.", message.author.avatarURL({
        dynamic: true
      })));
      if (users.length > 10)
      ...
      let embed = new MessageEmbed()
        ...
        .addField("<:asgard_mangekyou_sharingan:939963043750756443> Kişiler", message.guild.members.cache.filter(r => users.some(d => d == r.id)).map(r => r.id).map((r, index) => (index + 1) + "- " + message.guild.members.cache.get(r).toString()).join("\n"))
      return await message.channel.send(embed);
    }
    if (seçim == "limit") {
      let limit = args[1];
