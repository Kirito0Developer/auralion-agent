...
exports.run = async (client, message, args) => {
  ...
  if (seçim == "bilgi") {
    ...
    let embed = new MessageEmbed()
      ...
      .addField(`<:asgard_dev:939961596329988176> **Nasıl kullanılır?**`, `Jail Atmak için \`${prefix[0]}jail @etiket [sebep]\` komutunu kullan.`)
      .addField(`<:asgard_mavi_parlak_yildiz:939958604532940871> **Cezayı nasıl kaldıracağım?**`, `aili kaldırmak için \`${prefix[0]}unjail @etiket\` komutunu kullan.`)
      .addField("<:asgard_kalkan:939961737296347197> **Komutu kimler kullanabilir?**", "Bu komutu **Yönetici** ya da **Jail Sorumlusu** yetkisine sahip olan kişiler kullanabilir.")
      ...
      .addField(`<:asgard_beyaz_ok:939959055202549780> **Belirlediğim kanala loglatabiliyor muyum?**`, `Eğer belirlediğin kanala loglatmak isteiyorsan bunun için \n\`${prefix[0]}jail kanal [kanal/id]\` komutunu kullanmalısın.`)
  ...
  if (message.member.permissions.has("ADMINISTRATOR") && seçim != "bilgi") {
    if (seçim == "yetkiler") {
      ...
      if (roles.length <= 0)
        return await message.channel.send(new MessageEmbed()
          ...
          .addField(`<:asgard_boostlogo:940205843004141588> Örnek`, `\`\`\`${prefix[0]}jail yetkiler @rol1 @rol2 @rol3\`\`\``, true)
          .setThumbnail("https://cdn.discordapp.com/attachments/698398041518112898/699064766626267188/giphy.gif")
          .addField(`<:asgard_boostlogo:940205843004141588> Örnek`, `\`\`\`${prefix[0]}jail yetkiler ID1 ID2 ID3\`\`\``, true)
          .setFooter(message.author.username + " Tarafından kullanıldı.", message.author.avatarURL({
            dynamic: true
          })));
      ...
      let embed = new MessageEmbed()
        ...
        .addField("<:asgard_boostlogo:940205843004141588> Yetkiler", message.guild.roles.cache.filter(r => roles.some(d => d == r.id)).map(r => r.id).map((r, index) => (index + 1) + "- " + message.guild.roles.cache.get(r).toString()).join("\n"))
      return await message.channel.send(embed);
    }
    if (seçim == "kanal") {
      ...
      .addField(`<:asgard_boostlogo:940205843004141588> Örnek`, `\`\`\`${prefix[0]}jail kanal #kanal\`\`\``, true)
      .setThumbnail("https://cdn.discordapp.com/attachments/698398041518112898/699064766626267188/giphy.gif")
      .addField(`<:asgard_boostlogo:940205843004141588> Örnek`, `\`\`\`${prefix[0]}jail kanal ID\`\`\``, true)
      .setFooter(message.author.username + " Tarafından kullanıldı.", message.author.avatarURL({
        dynamic: true
      })));
    ...
    if (seçim == "kişiler") {
      ...
      .addField(`<:asgard_boostlogo:940205843004141588> Örnek`, `\`\`\`${prefix[0]}jail kişiler @etiket1 @etiket2\`\`\``, true)
      .setThumbnail("https://cdn.discordapp.com/attachments/698398041518112898/699064766626267188/giphy.gif")
      .addField(`<:asgard_boostlogo:940205843004141588> Örnek`, `\`\`\`${prefix[0]}jail kişiler ID1 ID2\`\`\``, true)
      .setFooter(message.author.username + " Tarafından kullanıldı.", message.author.avatarURL({
        dynamic: true
      })));
      if (users.length > 10)
      ...
      let embed = new MessageEmbed()
        ...
        .addField("<:asgard_boostlogo:940205843004141588> Kişiler", message.guild.members.cache.filter(r => users.some(d => d == r.id)).map(r => r.id).map((r, index) => (index + 1) + "- " + message.guild.members.cache.get(r).toString()).join("\n"))
      return await message.channel.send(embed);
    }
    if (seçim == "limit") {
      let limit = args[1];
