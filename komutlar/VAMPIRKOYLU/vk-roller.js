...
exports.run = async function (client, message, args) {
	...
	let vkdurum = await db3.get(`vkdurum.${message.member.voice.channel.id}`)
	 if (vkdurum == "Aktif") {
		...
		let roller = ["vampir", "büyücü", "şerif", "köylü"];
		
		if (!args[0]) {
			return message.reply(`Lütfen geçerli bir rol belirtiniz (\`${prefix[0]}vkroller 2vampir 1büyücü 1şerif\`) gibi yapabilirsiniz`)
		}
