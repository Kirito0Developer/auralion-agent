...
*Bu işlemi gerçekleştirdikten sonra* \`${prefix[0]}koruma hepsini #logkanalı\` *şeklinde giriniz*`)
    ...
    if (sec == "hepsini") {
      ...
      if (args[1] === "sıfırla" || args[1] === "kapat" || args[1] === "durdur") {
        if (kanal2) return message.channel.send(`<:asgard_carpii:939960104759996547> Ayarlanmayan şeyi sıfırlayamazsın.`)
        await db.delete(`sunucuayar.bot_koruma`)
        await db.delete(`sunucuayar.sunucu_koruma`)
        await db.delete(`sunucuayar.kanal_koruma`)
        await db.delete(`sunucuayar.rol_koruma`)
      ...
      if (!args[1] || !kanal2 || args[1] <= message.guild.memberCount) return message.channel.send(`<:asgard_carpii:939960104759996547> Gerekli yerleri düzgün doldurmalısın! =>  \`${prefix[0]}koruma hepsini #kanal\``)
